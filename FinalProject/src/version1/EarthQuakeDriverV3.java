...
public class EarthQuakeDriverV3 {
...
//		readFile(events, sigMonth);
		...
		try {
		    // Use JSoup to fetch raw JSON text from USGS
		    Document doc = Jsoup.connect("https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/significant_month.geojson")
		                        .ignoreContentType(true)
		                        .get();

		    String jsonText = doc.body().text();
		    readFromJson(events, jsonText);

		} catch (IOException e) {
		    System.out.println("Error retrieving data from the internet: " + e.getMessage());
		}
...
//				        count++;
	...
	private static void readFromScanner(ArrayList<Event> events, Scanner reader) {
	    double mag = 0;
	    String place = "";
	    double time = 0;
	    double updated = 0;
	    ...
	    while (reader.hasNextLine()) {
	        ...
	        if (insideFeatures) {
	            if (line.startsWith("\"mag\"")) {
	                ...
	                String coordLine = line.substring(line.indexOf("[") + 1, line.indexOf("]"));
	                String[] parts = coordLine.split(",");
	                longitude = Double.parseDouble(parts[0].trim());
	                latitude = Double.parseDouble(parts[1].trim());
	                depth = Double.parseDouble(parts[2].trim());
	...
	private static void readFromJson(ArrayList<Event> events, String jsonText) {
	    ...
	    for (int i = 0; i < features.length(); i++) {
	        ...
	        double time = props.optDouble("time", 0);
	        double updated = props.optDouble("updated", 0);
	        String url = props.optString("url", "");
	        String title = props.optString("title", "");
	        String geoType = geom.optString("type", "");
	        
	        JSONArray coords = geom.getJSONArray("coordinates");
	        double longitude = coords.optDouble(0);
	        double latitude = coords.optDouble(1);
	        double depth = coords.optDouble(2);
